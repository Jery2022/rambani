...
class UserService {
    static async getUserProfile(userId) {
        ...
        if (!user) {
            return { success: false, status: 404, message: 'Profil utilisateur non trouvé.' };
        }
        return { success: true, status: 200, profile: user };
    }

    static async updateUserProfile(userId, updateData, file) {
        let profilePicturePath = file ? `/images/profile_pictures/${file.filename}` : null;

        // Vérifier si le nom d'utilisateur est déjà pris par un autre utilisateur
        const existingUserByUsername = await UserModel.findByUsernameExcludingId(updateData.username, userId);
        if (existingUserByUsername) {
            if (file) fs.unlinkSync(file.path);
            return { success: false, status: 409, message: 'Ce nom d\'utilisateur est déjà pris.' };
        }

        // Vérifier si l'email est déjà pris par un autre utilisateur
        const existingUserByEmail = await UserModel.findByEmailExcludingId(updateData.email, userId);
        if (existingUserByEmail) {
            if (file) fs.unlinkSync(file.path);
            return { success: false, status: 409, message: 'Cet email est déjà utilisé par un autre utilisateur.' };
        }
        ...
        return { success: true, status: 200, message: 'Profil mis à jour avec succès.', updatedData: finalUpdateData };
    }
    ...
    static async createUser(username, password, email, role) {
        ...
        if (existingUserByUsername) {
            return { success: false, status: 409, message: 'Ce pseudo est déjà pris.' };
        }

        // Vérifier l'unicité de l'email
        const existingUserByEmail = await UserModel.findByEmail(email);
        if (existingUserByEmail) {
            return { success: false, status: 409, message: 'Cet email est déjà utilisé.' };
        }

        const hashedPassword = await bcrypt.hash(password, 10);
        await UserModel.create({ username, password: hashedPassword, email, role });

        return { success: true, status: 201, message: 'Utilisateur enregistré avec succès.' };
    }
    ...
    static async deleteUser(id) {
        ...
        return { success: true, status: 200, message: 'Utilisateur supprimé avec succès.' };
    }
    ...
    static async updateUser(id, updateData) {
        ...
        if (existingUserByUsername) {
            return { success: false, status: 409, message: 'Ce pseudo est déjà pris par un autre utilisateur.' };
        }
        ...
        return { success: true, status: 200, message: 'Utilisateur mis à jour avec succès.' };
    }
}
